...
class FeedTest {
    ...
    class Update {
        ...
        void update_Normal() {
            ...
            Feed feed = new Feed(contents, user);
            // when
            feed.update(updateContentsString);
            // then
            assertThat(feed.getId()).isNull();
            assertThat(feed.getContents()).isEqualTo(updateContentsString);
            assertThat(feed.getUser()).isEqualTo(user);
        }
            ...
            class Contents {
                ...
                void fail_null() {
                    ...
                    Exception exception = assertThrows(ApiRequestException.class,
                            () -> feed.update(updateContentsString));
                    // then
                    assertThat(exception.getMessage()).isEqualTo("공백으로 피드를 수정할 수 없습니다.");
                }
